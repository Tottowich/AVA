...
if record
    ...
    MOVE.FrameRate = 60; % Set frames per seond to PAL standard (animation speed)
    open(MOVE);
end
set(gca,'nextplot','replacechildren');

% Set the limits to avoid window stuttering.
minminx = min([min(X_marble(1,:,1),[],'all'),min(X(:,:,1),[],'all')]);
minminy = min([min(X_marble(1,:,2),[],'all'),min(X(:,:,2),[],'all')]);
minminz = min([min(X_marble(1,:,3),[],'all'),min(X(:,:,3),[],'all')]);
maxmaxx = max([max(X_marble(1,:,1),[],'all'),max(X(:,:,1),[],'all')]);
maxmaxy = max([max(X_marble(1,:,2),[],'all'),max(X(:,:,2),[],'all')]);
maxmaxz = max([max(X_marble(1,:,3),[],'all'),max(X(:,:,3),[],'all')]);
...
for n = 1:NM % Number of marbles
    ...
    r = X_marble(n,n,end);
    spheres(n) = surf(sx*r+pos(1),sy*r+pos(2),sz*r+pos(3),'EdgeColor','k','EdgeAlpha',0.5,'FaceColor',rand(1,3),'FaceAlpha',0.2);
